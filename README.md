...
- Trained on TinyStories through a learning-rate sweep, reaching a
  [validation loss of **1.62**](https://wandb.ai/sft_llm/stf-assignment1-basics-cs336_basics/sweeps/ww6pyabx?nw=nwuserzehlu); followed up with architecture ablations
  ([removing normalization](https://wandb.ai/sft_llm/stf-assignment1-basics-cs336_basics/sweeps/imemoybr?nw=nwuserzehlu), [pre-norm vs. post-norm](https://wandb.ai/sft_llm/stf-assignment1-basics-cs336_basics/runs/30labeh6?nw=nwuserzehlu)) to confirm their
  impact, tracked with [Weights & Biases](https://wandb.ai/sft_llm/stf-assignment1-basics-cs336_basics).
- Profiled and optimized the BPE tokenizer trainer (cProfile, scalene)
  for a **~3x** training-time speedup. See
  [BPE Tokenizer: 4 Optimization Rounds](docs/blog/tokenizer-performance.md),
  raw evidence in [`cs336_basics/profile/writeup/`](cs336_basics/profile/writeup/).
- Automated deployment (GitHub Actions CI/CD) of the tokenizer's
  training/encoding pipeline (Docker) to both **GCP** (Vertex AI, GCS)
  and **Azure** (ACA Jobs, ACR, Bicep IaC). See
  [Training the BPE Tokenizer in the Cloud](docs/blog/tokenizer-cloud-training.md),
  configs in [`deploy/gcp/`](deploy/gcp/) and
  [`cs336_basics/archive_solution/`](cs336_basics/archive_solution/).
