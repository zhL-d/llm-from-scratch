...
from cs336_basics.training_together import TrainConfig
...
def tokenize_and_save(cfg: TrainConfig):
    ...
    validate_corpus = cfg.data_vali_path.read_text(encoding="utf-8", errors="surrogatepass")
    token_ids = tokenizer.encode(validate_corpus)

    token_ids_ndarray = np.array(token_ids)
    
    np.save(cfg.tokenids_vali_path, token_ids_ndarray)
...
def validate_loop():
    ...
    checkpint_path = RUN_DIR / "last_checkpoint.pt"
    ...
    iter_step = load_checkpoint(checkpint_path, transformerlm, optimizer)
    ...
    for t in range(cfg.steps):
        ...
        validation_data: Int[Tensor, " batch_size context_length"] = data_batch_tuple[0]
        target_data: Int[Tensor, " batch_size context_length"] = data_batch_tuple[1]
    
        logit: Float[Tensor, " batch_size context_length vocab_size"] = transformerlm.forward(validation_data)
